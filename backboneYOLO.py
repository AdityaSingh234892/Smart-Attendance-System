...
def calculate_room_filled_capacity():
    # Define the room capacity
    room_capacity = 15

    # Calculate the room filled capacity
    room_filled_capacity = (count_var / room_capacity) * 100 if room_capacity > 0 else 0

    return room_filled_capacity

@app.route('/get_room_data')
def get_room_data():
    return jsonify(room_filled_capacity=calculate_room_filled_capacity(), number_of_people=count_var)
...
def classroom():
    ...
    room_capacity = 15  # Define the room capacity here
    if request.method == 'POST':
        
        if (request.form['ip'] == ''):
            inv_feed ="No Video-Feed!"
            return render_template('classroom.html', var2 = inv_feed, room_filled_capacity=calculate_room_filled_capacity(), room_capacity=room_capacity)
        
        else:
            ip_address = request.form['ip']
            ip_vd_feed = "Video-Feed"
            return render_template('classroom.html', ip_address = ip_address, var2 = ip_vd_feed, room_filled_capacity=calculate_room_filled_capacity(), room_capacity=room_capacity)
    
    if request.method == 'GET':
        return render_template('classroom.html', room_filled_capacity=calculate_room_filled_capacity(), room_capacity=room_capacity)
